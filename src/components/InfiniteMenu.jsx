...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
      <canvas
        ref={canvasRef}
        style={{ width: '100%', height: '100%', display: 'block', cursor: isMoving ? 'grabbing' : 'grab', outline: 'none' }}
      />

      {activeItem && (
        <>
          {/* Titre - À gauche sur desktop */}
          <h2 
            style={{
              position: 'absolute', top: '50%', left: '8%', transform: 'translateY(-50%)',
              fontWeight: 900, fontSize: 'clamp(1.8rem, 4vw, 3rem)', color: '#fff',
              margin: 0, pointerEvents: 'none', select: 'none',
              opacity: isMoving ? 0 : 1, transition: 'all 0.4s ease'
            }}
          >
            {activeItem.title}
          </h2>

          {/* Description - À droite sur desktop */}
          <p 
            style={{
              position: 'absolute', top: '50%', right: '8%', transform: 'translateY(-50%)',
              fontSize: 'clamp(0.9rem, 1.5vw, 1.2rem)', color: 'rgba(255,255,255,0.8)',
              maxWidth: '280px', margin: 0, pointerEvents: 'none', select: 'none',
              opacity: isMoving ? 0 : 1, transition: 'all 0.4s ease'
            }}
          >
          ...
          <button 
            ...
            style={{
              ...
              width: '60px', height: '60px', display: 'grid', placeItems: 'center',
              backgroundColor: '#5227ff', border: '4px solid #000', borderRadius: '50%',
              cursor: 'pointer', zIndex: 30, transition: 'all 0.4s ease'
            }}
          >
      ...
      {/* Styles d'affichage responsives pour masquer le texte sur petits écrans */}
      <style>{`
        @media (max-width: 1024px) {
          h2, p { display: none !important; }
        }
      `}</style>
    </div>
  );
